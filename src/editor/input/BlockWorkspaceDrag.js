...
export class BlockWorkspaceDrag {
  constructor(
    ...
    if (options.commit_StackSnap != null){
      this.commit_StackSnap = options.commit_StackSnap
    } else {
      this.commit_StackSnap = null
    }
  ...
  #onGrabStart(grabDetail){
    ...
    if (!StackChainDrag.is_StackHead(grabbedBlock)){
      const splitResult = StackChainDrag.split_StackAtGrabbed(
        this.blockRegistry,
        grabbedBlock
      )
      if (!splitResult){
    ...
    const stackChain =
      StackChainDrag.collect_ChainTree(
        this.blockRegistry,
        stackHead
      )
    ...
    const containerRect = SvgUtils.get_Rect(this.blockContainerEl)
    const overlayRect = SvgUtils.get_Rect(this.dragOverlayEl)
    ...
    const chainMembers = stackChain.map(chainBlock => {
      ...
      const { x: originX, y: originY } =
        SvgUtils.read_Transform(element)
      return {
        block: chainBlock,
        element,
        originX,
  ...
  #onGrabEnd(grabDetail){
    ...
    if (typeof this.commit_StackSnap === 'function'){
      snapPosition = this.commit_StackSnap(
        this.dragging,
        this.grabManager
      )
    }
    ...
    const headSnapped = Grid.calc_GridSnap(
      headBaseX,
      headBaseY
    )
