// --- Stack snap sockets (below / above) in client space ---
...
export function getStackSocketBelow(anchorBlock, blockRegistry) {
  const bottomZone = ConnectorZoneModule.ConnectorZone.zoneByType(anchorBlock.connectorZones, 'bottom');
...
export function getStackSocketAbove(anchorBlock, blockRegistry) {
  const topZone = ConnectorZoneModule.ConnectorZone.zoneByType(anchorBlock.connectorZones, 'top');
...
/** Dragged `<g>` bbox ∩ zone; for middle — live seam band (`middlePair`). */
export function draggedOutlineIntersectsSocket(draggedBlock, resolvedSocket) {
  ...
  if (resolvedSocket.zone?.type === 'middle' && resolvedSocket.middlePair) {
    ...
    socketClientRect = ConnectorClientGeometry.zoneToClientRect(resolvedSocket.el, resolvedSocket.zone);
  }
  if (!socketClientRect) return false;
  return ConnectorClientGeometry.rectsIntersectClient(draggedClientRect, socketClientRect);
}
