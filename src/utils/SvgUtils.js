...
export function createElement(tag, attrs = {}) {
  const element = document.createElementNS(Global.SVG_NS, tag)
...
export function read_UUID(element){
  if (!element || typeof element.getAttribute !== 'function') return ''
  return element.getAttribute(ATTR_WORKSPACE_BLOCK_UUID) || ''
}

export function read_Transform(element){
...
export function get_BBox(element){
...
export function get_Height(element, fallback = 0){
  const bbox = get_BBox(element)
...
export function get_Rect(element){
...
export function to_Local(element, clientX, clientY){
